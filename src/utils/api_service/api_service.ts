...
const unsafeGet = async <T = any>(url: string) => {
  return axios.get<T, any>(process.env.REACT_APP_API_URL + url, {
    headers: {
...
const get = async <U extends GetMethod["url"]>(url: U): Promise<AxiosResponse<ExtractResponse<U>, any>> => {
  return axios.get<ExtractResponse<U>, any>(process.env.REACT_APP_API_URL + url, {
    headers: {
      Authentication: `Bearer ${GlobalState.authToken.get()}`,
    },
  });
...
const unsafePost = async <T, U>(url: string, data: U) => {
  return axios.post<T, any>(process.env.REACT_APP_API_URL + url, data, {
    headers: {
      Authentication: `Bearer ${GlobalState.authToken.get()}`,
    },
  });
...
const post = async <U extends PostMethod["url"], K extends PostMethod["data"]>(
  ...
  return axios.post<ExtractResponse<U>, any>(process.env.REACT_APP_API_URL + url, data, {
    headers: {
      Authentication: `Bearer ${GlobalState.authToken.get()}`,
    },
  });
